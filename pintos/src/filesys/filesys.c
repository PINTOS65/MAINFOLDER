...
filesys_remove (const char *name) 
{
  ...
  success = success && dir_lookup (dir, parsed_name, &inode);

  if (success)
  {
    if (dir_entry_is_dir (dir, inode_get_inumber (inode)))
    {
      struct dir* temp;
      char dummy[NAME_MAX + 1];

      if (dir_readdir (temp = dir_open (inode), dummy))
        success = false;

      /* checking whether the victim directory is being used */
      if (inode_is_used (inode) || thread_on_dir (inode_get_inumber (inode)))
        success = false;
...
filesys_parse (const char* name_, struct dir** dir, char* dst, bool* is_dir)
{
  ...
  *dir = dir_open (inode_open (inode_get_parent (start)));
  ...
  {
    if (strlen (token) > NAME_MAX) //filename length limit
    {
      success = false;
      break;
    }
  ...
  if (success && dst_ptr != NULL)
  {
    memcpy (dst, dst_ptr, NAME_MAX);
    dst[NAME_MAX] = '\0';
  }
